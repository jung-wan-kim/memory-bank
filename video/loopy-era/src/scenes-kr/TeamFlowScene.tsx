...
const PHASES: Phase[] = [
  { id: 0, label: "Phase 0", title: "Plan",    color: colors.muted,   병렬: false },
  { id: 1, label: "Phase 1", title: "설계",  color: colors.blue,    병렬: false },
  { id: 2, label: "Phase 2", title: "DB/API",  color: colors.green,   병렬: false },
  {
    id: 3,
    label: "Phase 3",
    title: "구현",
    color: colors.accent,
    병렬: true,
    병렬Lanes: ["Frontend", "Backend"],
  },
  { id: 4, label: "Phase 4", title: "Test",    color: colors.orange,  병렬: false },
  { id: 5, label: "Phase 5", title: "Ship",    color: colors.cyan,    병렬: false },
];
  ...
  return (
    <AbsoluteFill style={{ background: colors.bg, fontFamily: sans }}>
      ...
      {PHASES.map((phase, i) => {
        ...
        const isParallel = phase.병렬 && phase.병렬Lanes;
      ...
      >
        <div style={{ fontSize: 15, color: colors.muted }}>
          Phase 3 병렬izes Frontend + Backend — cutting implementation time in half
        </div>
      </div>
    </AbsoluteFill>
  );
