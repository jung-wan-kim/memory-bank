...
const PIPELINE1: PipelineNode[] = [
  { label: "대화", sublabel: "JSONL", color: colors.blue },
  { label: "파서", sublabel: "extract exchanges", color: colors.blue },
  { label: "임베딩", sublabel: "384차원", color: colors.cyan },
...
const PIPELINE2: PipelineNode[] = [
  { label: "세션 종료", sublabel: "hook trigger", color: colors.orange },
  { label: "Haiku LLM", sublabel: "fact extraction", color: colors.orange },
  { label: "통합", sublabel: "dedup + merge", color: colors.red },
  { label: "팩트 DB", sublabel: "2,600+", color: colors.accent },
...
export const DataPipelineScene = () => {
  ...
  return (
    <AbsoluteFill style={{ background: colors.bg, fontFamily: sans }}>
      ...
      {/* 통합 type labels below pipeline 2 */}
      <div
        style={{
          position: "absolute",
          left: START_X,
      ...
      >
        <div style={{ fontSize: 14, color: colors.muted }}>
          Two pipelines run in 병렬 — every session grows the knowledge base
        </div>
      </div>
    </AbsoluteFill>
  );
